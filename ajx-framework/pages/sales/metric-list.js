...
$(function(){
    ...
    // Draw Metric
    function metricTableView(selector,d,onclick,ondblclick)
    {  var s = '';
       var i;
       var edit = [false,true,true,false];
       var names = [,'metric','description']
       // name="me" 
       ...
       {   var j;
           ...
           s+='<td><div class="btn-group  pull-right"><button class="btn btn-sm"><a target="_blank" href="/html.php/pages/sales/metric.csv?id='+r.id+'">Download CSV</a></button>'+
       ...
       {   var id = $(row.target).parents('tr:first').addClass('active').attr('data-id');
           var r = d.rows[id];
           if (confirm('Delete '+r.metric+' ('+r.created+') ?')) 
           {  console.log('delete', r.id);
              ajx('/pages/sales/Model/metrics/delete', {id:r.id}, function(d){
                   if (!d.error) 
                   { setOk('Row Deleted');
                     model.load();
                   }
       ...
       $(selector+' tbody tr td').blur(function(e){
            ...
            {  // value  changed 
               ...
               r[name] = value;               
               ajx('/pages/sales/Model/metrics/update', r, function(d){                   
